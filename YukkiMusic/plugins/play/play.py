...
import config
from config import BANNED_USERS, lyrical, NAME_AMR
...
@app.on_message(command([f"{NAME_AMR} {PLAY}", "شغلي", "تشغيل", "شغل"]) & filters.group & ~filters.edited& ~BANNED_USERS
)
@PlayWrapper
async def play_commnd(
    ...
    mystic = await message.reply_text(
        _["play_2"].format(channel) if channel else "**• من عيوني جاري التشغيل ..**"
    )
    plist_id = None
    slider = None
    plist_type = None
